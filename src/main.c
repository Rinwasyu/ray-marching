...
int main(void) {
	...
	s.spheres[0] = Sphere(Vec3(0, 4, 0), 3, 0, 0, 0, Rgb(0,1,0));
	s.spheres[1] = Sphere(Vec3(0, 2, 5), 2, 0, 0, 0, Rgb(0,0,1));
	s.spheres_cnt = 2;
	s.boxes[0] = Box(Vec3(0, -0.25, 0), Vec3(10, 0.5, 10), 0, 0, 0, Rgb(1,1,0));
	s.boxes[1] = Box(Vec3(7, 2.5, 0), Vec3(1, 2.5, 1), 0, 0, 0, Rgb(1,0,1));
	s.boxes[2] = Box(Vec3(7, 5, -7), Vec3(0.1, 5, 0.1), 0, 0, 0, Rgb(1,1,1));
	s.boxes_cnt = 3;
	s.toruses[0] = Torus(Vec3(7, 2.5, 7), Vec2(1.5, 0.8), PI/2+PI/5, -PI/3, 0, Rgb(1,1,0));
	s.toruses_cnt = 1;
